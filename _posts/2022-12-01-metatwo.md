...
We are able to login, but it doesn't seem like we have access to do very much. There is one interesting thing that catches my eye right away though, we can upload new media. The first thing I did was try uploading .php and .phar files, but it looks like we are restricted on what kind of files we can upload.
...
In the [github for `passpie`](https://github.com/marcwebbie/passpie), we can see all of the commands we can run in passpie and looks like we can export the password for root by running `passpie export root`.
